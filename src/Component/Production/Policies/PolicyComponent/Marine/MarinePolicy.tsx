...
import { UpwardTableModalSearch, useUpwardTableModalSearchSafeMode } from "../../../../DataGridViewReact";
import {
...
export default function MarinePolicy() {
  ...
  const { isPending: isLoadingAccount, mutate: mutateAccount } = useMutation({
    ...
    mutationFn: (variables: any) => {
      return myAxios.post("/task/production/marine/get-account", variables, {
        headers: {
  ...
  const { isPending: isLoadingWords, mutate: mutateWord } = useMutation({
    mutationKey: ["words"],
    mutationFn: (variables: any) => {
      return myAxios.post("/task/production/marine/get-words", variables, {
        headers: {
          Authorization: `Bearer ${user?.accessToken}`,
        },
      });
  ...
  const mutateWordRef = useRef(mutateWord);
  useEffect(() => {
