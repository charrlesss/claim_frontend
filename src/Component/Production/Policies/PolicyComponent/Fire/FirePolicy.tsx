...
export default function FirePolicy() {
  ...
  const { isPending: isLoadingOccupancy, mutate: mutateOccupancy } =
    useMutation({
      mutationKey: ["occupancy"],
      mutationFn: (variables: any) => {
        return myAxios.post("/task/production/fire/get-occupancy", variables, {
          headers: {
  ...
  const { isPending: isLoadingAccount, mutate: mutateAccount } = useMutation({
    mutationKey: ["account"],
    mutationFn: (variables: any) => {
      return myAxios.post("/task/production/fire/get-account", variables, {
        headers: {
          Authorization: `Bearer ${user?.accessToken}`,
        },
      });
  ...
  const { isPending: isLoadingMortgagee, mutate: mutateMortgagee } =
    useMutation({
      ...
      mutationFn: (variables: any) => {
        return myAxios.post("/task/production/fire/get-mortgagee", variables, {
          headers: {
            Authorization: `Bearer ${user?.accessToken}`,
          },
        });
  ...
  const mutateOccupancyRef = useRef(mutateOccupancy);

  useEffect(() => {
    mutateMortgageeRef.current({});
    mutateAccountRef.current({});
    mutateOccupancyRef.current({});
  }, []);
