...
export default function BondsPolicy() {
  ...
  const { isPending: isLoadingBondSubline, mutate: mutateBondSubline } =
    useMutation({
      mutationKey: ["bond-subline"],
      mutationFn: (variables: any) => {
        return myAxios.post(
          "/task/production/bond/get-bond-subline",
          variables,
          {
            headers: {
  ...
  const { mutate: mutateAccount, isPending: isLoadingAccount } = useMutation({
  ...
  const mutateBondSublineRef = useRef<any>(mutateBondSubline);
  const mutateAccountRef = useRef<any>(mutateAccount);

  useEffect(() => {
    mutateAccountRef.current({ policyType: "" });
    mutateBondSublineRef.current({});
  }, []);
