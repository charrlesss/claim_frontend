...
import { UpwardTableModalSearch } from "../../../../DataGridViewReact";
...
function COMPolicy({ user, myAxios, policy, setPolicy, _policy }: any) {
    const { subAccountData } = useContext(PolicyContext);
...
const COMRegular = forwardRef(
  (
    ...
    const { mutate: mutatateAccount, isPending: isLoadingAccount } =
      useMutation({
        mutationKey: ["account"],
    ...
    const { mutate: mutatateMortgagee, isPending: isLoadingMortgagee } =
      useMutation({
    ...
    const { mutate: mutatateDenomination, isPending: isLoadingDenomination } =
      useMutation({
    ...
    const {
      ...
      isPending: isLoadingSelectedSearch,
    } = useMutation({
    ...
    const { mutate: mutatateSave, isPending: isLoadingSave } = useMutation({
      mutationKey: ["save"],
      mutationFn: (variables: any) => {
    ...
    const { mutate: mutatateUpdate, isPending: isLoadingUpdate } = useMutation({
      mutationKey: ["update"],
      mutationFn: (variables: any) => {
        const link =
          policy === "COM"
...
const COMTemporary = forwardRef(
  (
    ...
    const { isPending: isLoadingTempId, mutate: mutateTempID } = useMutation({
      mutationKey: ["temp-id"],
      mutationFn: () => {
